...
public class SimpleCrewTeamAssignSubmit extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        final DbConnect dbc1 = dbm.doQuery("SELECT rover_id FROM rover WHERE valid='Y' ORDER BY rover_crew, rover_id");
        ...
        for(int i=0; i<team_count; i++) {
            for(int j=0; j<team_size; j++) {
                int idx = i*team_size+j;
                if(idx < count) {
                    dbc2.setInt(1, roverList.get(idx));
                    dbc2.setInt(2, i+1);
